...
public:
    ...
    int getMinuutit() const {
        return minuutit;
    }

    bool aikaisempiKuin(const Aika &toinen) const {
        return (tunnit < toinen.getTunnit()) ||
               (tunnit == toinen.getTunnit() && minuutit < toinen.getMinuutit());
    }

    Aika vahenna(const Aika &toinen) const {
        const int kokonaisMinuutit1 = tunnit * 60 + minuutit;
        const int kokonaisMinuutit2 = toinen.getTunnit() * 60 + toinen.getMinuutit();
